...
module.exports = class PurgeCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    if (!args[0]) return message.channel.send("You have to state how many messages i have to purge. \`h!purge number\`");
    const amountToDelete = Number(args[0], 10);

    if (isNaN(amountToDelete)) return message.channel.send("The number stated is not a valid number.");
    if (!Number.isInteger(amountToDelete)) return message.channel.send("Number must be a whole number.");
    if (!amountToDelete || amountToDelete < 2 || amountToDelete > 100) return message.channel.send('The number has to be between 2 and 100.');
    ...
    try {
      ...
      console.Log(err);
      message.channel.send(`I was unable to delete the amount of messages stated. Make sure they are within 14 days old.`);
    }
  }
}
